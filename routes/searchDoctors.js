...
router.post('/searchDoctors',async (req,res,next)=>{
    var username="",speciality="",gender="",yoe=0,qualification="",state="",city="";
    ...
    try{
            ...
            let doctor_exist=await Doctor.find({ username: { $regex:".*"+username+".*",$options:'i' },speciality: { $regex:".*"+speciality+".*",$options:'i' }, yoe : { $gte:yoe},gender:{ $regex:`^${gender}`,$options:'i' },qualification:{ $regex:".*"+qualification+".*",$options:'i' },state:{ $regex:".*"+state+".*",$options:'i' },city:{ $regex:".*"+city+".*",$options:'i' }});
